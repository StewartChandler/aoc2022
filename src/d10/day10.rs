...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    print!("Part 2: ");
    (0..6)
        .flat_map(|y| (0..40).zip(std::iter::repeat(y)))
        .for_each(|(x, y)| {
            let c = y * 40 + x;
            let m = if x_vals[c] > 0 {
                7u64.wrapping_shl((x_vals[c] - 1) as u32)
            } else {
                7u64.wrapping_shr((1 - x_vals[c]) as u32)
            };

            let s = if ((1 << x) & m) != 0 { "#" } else { "." };
